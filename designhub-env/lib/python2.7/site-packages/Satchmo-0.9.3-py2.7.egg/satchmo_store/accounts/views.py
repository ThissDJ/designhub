...
def register_handle_form(request, redirect=None, invitor_id=None):
    """
    Handle all registration logic.  This is broken out from "register" to allow easy overriding/hooks
    such as a combined login/register page.
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            contact = form.save(request, force_new=True)
            # send the signal that an invitee is invited by invitor with this id 
            if not invitor_id is None:
                invitor = Contact.objects.filter(id = invitor_id)
                if invitor.count():
                    invitor = invitor[0]
                    signals.new_contact_created.send(sender=request, invitor=invitor, invitee=contact)
        ...
        initial_data[redirect_field_name] = redirect_to
...
# 
from localsite.models import InviteReward
from satchmo_store.accounts.mail import send_reward_email
import sys
def log_new_contact_created(sender, **kwargs):

    invitor = kwargs.get('invitor','')
    invitee = kwargs.get('invitee','')
    if invitor and invitee:
        inviteReward = InviteReward.objects.filter(invitor = invitor)
#        print >>sys.stderr, 'Goodbye, cruel world! There is such invitor %s %s' %(invitor.first_name,invitor.last_name)
        if inviteReward.count():
            inviteReward = inviteReward[0]
        else:
            inviteReward = InviteReward(invitor = invitor)
            inviteReward.save()
        inviteReward.invitees.add(invitee)
    else:
        pass
#        print >>sys.stderr, 'Goodbye, cruel world! There is not such invitor'


signals.new_contact_created.connect(log_new_contact_created)
...
m2m_changed.connect(invitees_changed, sender=InviteReward.invitees.through)
#---- Views
...
INVITOR_FIELD_NAME = 'invitor'
def register(request, redirect=None, template='registration/registration_form.html'):
    """
    Allows a new user to register an account.
    """
    invitor_field_name = INVITOR_FIELD_NAME
    redirect_to = request.REQUEST.get(redirect_field_name, '')
    # invitor check
    invitor_id = request.REQUEST.get(invitor_field_name, None)
    def invitor_id_valid(s):
        try: 
            int(s)
            if int(s) > 0:
                return True
            ...
            return False
    if not(invitor_id is None) and invitor_id_valid(invitor_id):
        invitor_id = int(invitor_id)
        template = 'registration/registration_form_invite.html'
    else:
        invitor_id = None
    ret = register_handle_form(request, redirect_to,invitor_id)
    success = ret[0]
    todo = ret[1]
    if len(ret) > 2:
        extra_context = ret[2]
    ...
    if success:
        ...
        context['is_register_only'] = True
        context[invitor_field_name] = invitor_id
