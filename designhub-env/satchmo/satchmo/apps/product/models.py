...
class Category(models.Model):
    ...
    def active_products_price_inc(self, variations=False, include_children=False, **kwargs):
        """Variations determines whether or not product variations are included
        in most templates we are not returning all variations, just the parent product.
        """
        if not include_children:
...
#            return qry.filter(site=self.site, active=True, productvariation__parent__isnull=True, **kwargs)
            pts = list(qry.filter(site=self.site, active=True, productvariation__parent__isnull=True, **kwargs))
            pps = []
            products = []
            for p in pts:
                pps.append([p.unit_price,p])
            pps.sort()
            for pp in pps:
                products.append(pp[1])
            return products
    
    def active_products_price_dec(self, variations=False, include_children=False, **kwargs):
        """Variations determines whether or not product variations are included
        in most templates we are not returning all variations, just the parent product.
        """
        if not include_children:
